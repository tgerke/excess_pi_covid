...
hhs_states <- hhs_states %>% 
  ...
  rename(state = state_abb,
         state_region = region) %>%
  mutate(state_region = str_replace(state_region, " ", ""))
...
# all state_region codes for state (or simply annotate plots with state)
...
hhs_states_current <- hhs_states_current %>%
  group_by(state_region) %>%
  mutate(state_region.lab = paste(unique(state), collapse = ",")) %>%
  ungroup()

# add a custom row for New York state
hhs_states_current <- hhs_states_current %>%
  add_row(state = "NYC", state_region = "NYC", state_region.lab = "NYC") 
```
...
clin$state <- state.abbs[match(clin$region, state.names)]
