...
end.data.date <- as.Date('2020-08-15')
# uncomment below for rendering in production
...
```{r setup}
library(tidyverse)
library(ExcessILI) # devtools::install_github("weinbergerlab/ExcessILI")
library(cdcfluview) # may only be using for mmwr_week()
library(reshape2) # used for melt(); will eliminate
library(lubridate)
library(htmlTable)
library(abind)
...
```{r statelookup}
# some resources use variations on state names/abbreviations
# define a more expansive state.names and state.abbs to handle
state.names <- 
  c(state.name, 
...
```{r states_to_plot}
plot.states <- 
  c("CA", "TX", "FL", "NJ", "NY", "WA", "IL", "GA", "MI", "LA", "MA")

exclude.states <- 
  c("CT", "NC", "PR") # no up-to-date number
...
# add state name abbreviations and specify that region is a state region
hhs_states <- hhs_states %>% 
  inner_join(state_lookup %>% rename(state_or_territory = state_name)) %>%
  rename(state = state_abb,
         hhs_region = region) %>%
  mutate(hhs_region = as.numeric(str_replace(hhs_region, "Region ", ""))) %>%
  select(state, hhs_region)
# earlier version of code renamed state_region to the state abbreviation 
# for states that are in plot.states. If an issue later, suggest swapping
# all hhs_region codes for state (or simply annotate plots with state)

# # Exclude CT, NC, PR
# hhs_states_current <- hhs_states %>%
#   filter(!state %in% exclude.states)

# create a region label with all included states
hhs_states <- hhs_states %>%
  group_by(hhs_region) %>%
  mutate(hhs_region.lab = paste(unique(state), collapse = ",")) %>%
  ungroup()
```

```{r nrevs_format}
nrevss.state <- 
  runIfExpired("nrevss_state", ~ cdcfluview::who_nrevss(region = c("state")))

# nrevvs_clin formerly called `clin` and `clin2`
nrevss_clin <- nrevss.state[["clinical_labs"]]
# regions are states; assign state abbreviations to these
nrevss_clin <- nrevss_clin %>%
  ...
  select(-region_type, -state_name, -percent_positive) %>%
  left_join(hhs_states, by = "state") # append HHS region info

## Florida doesn't have ILI data, so use regions ILI dat
nrevss_hhs <- 
  runIfExpired("nrevss_hhs", ~ cdcfluview::who_nrevss(region = c("hhs")))

nrevss_hhs_clin <- nrevss_hhs[["clinical_labs"]]
nrevss_hhs_clin <- nrevss_hhs_clin %>% 
  mutate(hhs_region = as.numeric(str_remove(region, "Region "))) %>%
  rename(hhs_total_a = total_a,
         hhs_total_b = total_b,
         hhs_total_specimens = total_specimens) %>%
  select(hhs_region, wk_date, hhs_total_a, hhs_total_b, hhs_total_specimens)

# Join and if no specimen data for state, use HHS region estimates
nrevss <- 
  full_join(nrevss_clin, nrevss_hhs_clin, 
                    by = c("hhs_region", "wk_date")) %>%
  mutate(total_specimens = coalesce(total_specimens, hhs_total_specimens),
         total_a = coalesce(total_a, hhs_total_a),
         total_b = coalesce(total_b, hhs_total_b),
         flu_pct_adj = (total_a + total_b) / total_specimens) %>%
  select(state, wk_date, flu_pct_adj) %>%
  group_by(state) %>% #create 7 and 14 day lags 
  mutate(flu_pct_adj_lag7 = lag(flu_pct_adj, 1),
         flu_pct_adj_lag14 = lag(flu_pct_adj, 2)) %>%
  ungroup() 

## national flu data
nrevss.natl <- 
  runIfExpired("nrevss_national", ~ cdcfluview::who_nrevss(region = c("national")))

nrevss_natl_clin <- nrevss.natl$clinical_labs
nrevss_natl_clin <- nrevss_natl_clin %>%
  mutate(state = "US") %>%
  select(state, wk_date, percent_positive) %>%
  rename(flu_pct_adj = percent_positive) %>%
  mutate(flu_pct_adj = flu_pct_adj / 100) %>%
  mutate(flu_pct_adj_lag7 = lag(flu_pct_adj, 1),
         flu_pct_adj_lag14 = lag(flu_pct_adj, 2))

nrevss <- bind_rows(nrevss_natl_clin, nrevss)
...
dimnames(delays)[[3]] <-
  state.abbs[match(dimnames(delays)[[3]], state.names)]
...
nchs.base1.data <-
  read.csv("./Data/nchs_base_csvs/Weekly_Counts_of_Deaths_by_State_and_Select_Causes__2014-2018.csv")
...
nchs.base2.data <- runIfExpired(
  "nchs_base2_data",
  ~ read.csv("https://data.cdc.gov/api/views/muzy-jte6/rows.csv?accessType=DOWNLOAD")
)
nchs.base2.data <- nchs.base2.data[, c("jurisdiction_of_occurrence", "week_ending_date", "all_cause")]
...
nchs.base2.data$week_end <- as_date(nchs.base2.data$week_end, "%m/%d/%Y")
...
pi.data.not.miss.states <- state.abbs[match(pi.data.not.miss.states, state.names)]
...
pi.data.exclude$state <-
  state.abbs[match(pi.data.exclude$state, state.names)]
...
natl.pi.data <-
  aggregate(
    pi.data.exclude[, c("pi.fv", "all_cause")],
    by = list(
      "week_end" = pi.data.exclude$week_end,
      "year_week_num" = pi.data.exclude$week
    ),
    FUN = sum
  )
...
cdc.comb2$state <-
  state.abbs[match(cdc.comb2$state, state.names)]

cdc.comb2 <- cdc.comb2[!(cdc.comb2$state %in% exclude.states), ]

cdc.data.ny.separate <- cdc.comb2

cdc.comb2 <- merge(cdc.comb2, hhs_states_current, by = "state", all = T)
...
min.state <- lapply(spl1, function(x) {
  x$miss.x <- min(x$total_pni, na.rm = T)
  return(x)
})
...
test.ds <- merge(test.ds, hhs_states_current, by = "state")
...
pop1$state <- state.abbs[match(pop1$state_name, state.names)]
...
pop1.reg <- merge(pop1, hhs_states_current, by = "state")
...
sum.cases.excess <- sapply(
  1:length(sum.obs.state),
  function(x) {
...
sum.ac.excess <- sapply(
  1:length(sum.obs.ac2),
  function(x) {
...
for (i in c("LA", "MD", "CO", "GA", "VA", "TX", "CA", "AZ", "FL")) {
  ...
  title(state.names[match(i, state.abbs)])
}
```
...
comb1 <- merge(
  res.ac2,
  res.pi2,
  by.x = c("state", "week_end_date", "week_start_date"),
  by.y = c("state", "week_end_date", "week_start"),
  all = T
)
...
formatted.excess.ac <-
  paste0(
    round(sum.excess.deaths.range.ac[, "50%"], -2),
    " (",
...
formatted.excess.pic <-
  paste0(
    round(sum.excess.deaths.range.pi[, "50%"], -1),
    " (",
...
formatted.excess.ac.INC <-
  paste0(
    round(sum.excess.deaths.range.ac.inc[, "50%"] / sum.excess.deaths.range.ac.inc[, "census_bureau_pop_2019"] * 100000, 1),
    " (",
...
for (i in plot.state.indices) {
  ...
  if (states[i] %in% state.abbs) {
    state.name.plot <-
      state.names[match(states[i], state.abbs)]
  } else {
    state.name.plot <- states[i]
  }
  plot(
    1:length(present.weeks),
    pred[i, , "2020"],
    type = "l",
    col = "red",
    ylim = y.range1,
...
for (i in 1:length(comb2.spl)) {
  ...
  state.name.plot <-
    state.names[match(unique(ds1$state), state.abbs)]

  plot(
    ds1$week_end_date,
    ds1$all_cause_deaths,
    type = "l",
    col = "black",
    ylim = y.range1,
    bty = "l",
...
anim <- p +
  ...
  ggtitle(
    "Now showing week ending {closest_state}",
...
ili.data$state <- state.abbs[match(ili.data$region, state.names)]
...
ili.data2 <- merge(ili.data, hhs_states_current, by = "state")
...
for (i in plot.state.indices) {
  y.range1 <- c(0, 7)
  if (dimnames(rr.common.states)[[2]][i] %in% state.abbs) {
    state.name.plot <-
      state.names[match(dimnames(rr.common.states)[[2]][i], state.abbs)]
  } else {
  ...
  points(
    dates.plot4[1:nrow(rr2.ili.comp)],
    exp(rr2.ili.comp[, i]),
    type = "l",
    col = "#377eb8",
    lty = 2
  ...
  if (i == plot.state.indices[1]) {
    legend(
      "topleft",
      legend = c("Outpatient ILI", "Excess deaths"),
      col = c("#377eb8", "#e41a1c"),
      inset = 0.01,
      lty = c(1, 3, 2),
      box.lty = 0,
...
### There was an extra } here previously
```
...
for (week.test in c("11", "23")) { # week for which CDCfluview is for
  provis.list <- lapply(
    c("2019-2020", "2018-2019", "2017-2018", "2016-2017"),
    function(x) {
  ...
  provis.data <-
    provis.data[
      as.character(provis.data$epiyr) ==
  ...
  provis.data.m <-
    melt(
      provis.data[, c("All.Deaths", "pi.deaths", "epiwk", "Week", "provision.year")],
...
###################
sum.cases.excess.adj <- sapply(
  1:length(sum.obs.state.adj),
  function(x) {
...
formatted.ac.adj <-
  paste0(
    round(sum.excess.deaths.range.ac.adj[, "50%"], -1),
    " (",
...
points(
  x2.agg$week_end_date,
  x2.agg$excess_all_cause_deaths,
  type = "l",
  col = "black"
)
